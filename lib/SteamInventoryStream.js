...
class SteamInventoryStream extends Readable {
  ...
  constructor (steamId, appId, contextId, language = 'en', maxSequentialErrors = 5, maxChunkSize = 5000) {
    ...
    if (!maxSequentialErrors) throw new Error('No maxSequentialErrors was provided')
    if (!maxChunkSize) throw new Error('No maxChunkSize was provided')
  ...
  getBatch () {
    return new Promise((resolve, reject) => {
      ...
      const qs = {
        l: this.language,
        start_assetid: this.lastAssetId,
        count: this.count
      }
      // Do the request
      request.get(
